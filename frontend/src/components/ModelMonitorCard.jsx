...
function getSummaryText(monitorStatus) {
  ...
  if (monitorStatus?.status === "fallback_only") return "当前未连上模型接口，系统会退回本地兜底逻辑。";
...
function InfoItem({ label, value }) {
  return (
    <div className="rounded-[18px] border border-slate-200 bg-slate-50/80 px-4 py-3">
      ...
      <p className="mt-2 text-sm font-semibold text-[var(--ink)] break-all">{value}</p>
    </div>
  );
}
